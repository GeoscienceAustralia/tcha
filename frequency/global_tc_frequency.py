...
def calculateFrequency(df):
    ...
    stormcount = pd.Series(index=range(ntcs.index.min(), ntcs.index.max()+1), dtype=int, data=0)
    stormcount.loc[ntcs.index] = np.array(ntcs.DISTURBANCE_ID.values, dtype='int32')
    return stormcount
...
def fitFrequency(tccount):
    ...
    plt.savefig(pjoin(
        OUTPUT_DIR, f"{basinname}.seasonal_frequency_posterior_predictive_mean.png"),
        bbox_inches='tight')
    mtrace.posterior_predictive['obs'][0].to_dataframe().to_csv(
        pjoin(OUTPUT_DIR, f"tccount.samples.{basin}.csv")
        )
