...
for row in list(df.itertuples())[:]:
    ...
    try:
        ...
        u = 0.95 * u_dlm.mean() - 3.987 + np.random.normal(loc=0.0, scale=6.71)
        v = 0.81 * v_dlm.mean() - 1.66 + np.random.normal(loc=0.0, scale=5.39)
...
df.to_csv(os.path.expanduser("~/geoscience/data/coarse_tc_tracks_stochastic.csv"))
