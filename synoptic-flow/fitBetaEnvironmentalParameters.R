...
df$dudy2 <- df$dudy**2
df$dvdx2 <- df$dvdx**2
lmu <- lm(ub ~ dudy + dvdx + dudy2 + dvdx2 + dzdy + su + sv + lat, data=df)
lmv <- lm(vb ~ dudy + dvdx + dudy2 + dvdx2 + dzdy + su + sv + lat, data=df)

bestlmu <- stepAIC(lmu, direction = "both")
bestlmv <- stepAIC(lmv, direction = "both")

predub <- predict(bestlmu)
residub <- residuals(bestlmu)

# Extract predicted values and residuals for Model 2
predvb <- predict(bestlmv)
residvb <- residuals(bestlmv)

par(mfrow = c(1, 2))

plot(predub, df$ub,
     xlab = expression(paste("Predicted values (u_beta)")),
